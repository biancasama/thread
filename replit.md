...
Thread is an AI-powered external working memory tool that helps users recover their train of thought after interruptions, multitasking, or context switching. It uses Gemini AI to transform raw thoughts into structured "thought threads".
...
- **AI**: Gemini via Replit AI Integrations (gemini-2.5-flash)
...
- **Capture Screen**: Text input to capture thoughts; Gemini parses them into structured threads
...
- `POST /api/thread/parse` — Sends text to Gemini, returns structured thread JSON
...
- System prompt instructs Gemini to parse thoughts into Thread JSON structure
